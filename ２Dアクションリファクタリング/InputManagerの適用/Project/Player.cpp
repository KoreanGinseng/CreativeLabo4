...
void CPlayer::UpdateKey(void){
	...
	//攻撃中は移動できないようにする
	if(InputManagerInstance.IsNegativePress("Horizontal"))
	{
	...
	else if(InputManagerInstance.IsPress("Horizontal"))
	{
		m_MoveX += PLAYER_SPEED;
		m_bReverse = false;
		m_bMove = true;
	...
	//上キーでジャンプ
	if(InputManagerInstance.IsPress("Jump") && !m_bJump)
	{
		m_bJump = true;
		m_MoveY = PLAYER_JUMP;
	...
	//SPACEキーで攻撃
	if(InputManagerInstance.IsPush("Horizontal"))
	{
