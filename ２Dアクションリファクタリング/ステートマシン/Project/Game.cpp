 ...
 */
CGame::CGame()
	: hpRender(std::make_shared<Sample::HPGauge>(100)) {
}
...
bool CGame::Load(void){
	...
	*/
	...
	//リソース
	auto& pack = ResourceManagerInstance.LoadPack("Player", "PlayerResources.json");

	//auto pack = Sample::JsonResourcePackCreator<Sample::Texture>::Create("PlayerResources.json");

	/*
	auto& pack = ResourceManagerInstance.AddPack("Player");
	auto tex = pack->Get<Sample::Texture>()->Add("Player");
	tex->Load("player.png");
	...
	*/
	...
	m_Player.Load(pack, Sample::JsonAnimationCreator::Create("PlayerAnimation.json"));
	hpRender.Load(pack);
	m_Player.GetHPSubject()->Subscribe(hpRender.GetGauge());
	//ステージの素材読み込み
...
void CGame::Update(void){
	...
	for (int i = 0; i < m_Stage.GetEnemyCount(); i++)
	{
		Sample::CollisionManager::Collision(m_Player, m_EnemyArray[i]);
	}
	for (int i = 0; i < m_Stage.GetItemCount(); i++)
	{
		Sample::CollisionManager::Collision(m_Player, m_ItemArray[i]);
	}
	//ステージの更新
...
void CGame::Render(void){
	//描画タスクの生成
	Sample::RenderCommandTaskPtr ptr = std::make_shared<Sample::RenderCommandTask>(2);
	//ステージの描画
	m_Stage.Render();
	//プレイヤーの描画
	Sample::PlayerRender::Register(&m_Player, m_Stage.GetScrollX(), m_Stage.GetScrollY(), ptr);
	//敵の描画
	for(int i = 0;i < m_Stage.GetEnemyCount();i++)
	{
...
void CGame::RenderDebug(void){
	//描画タスクの生成
	Sample::RenderCommandTaskPtr ptr = std::make_shared<Sample::RenderCommandTask>(1);
	//ステージのデバッグ描画
	m_Stage.RenderDebug();
	//プレイヤーのデバッグ描画
	Sample::PlayerDebugRender::RenderState(&m_Player, 10, 70, ptr);
	Sample::PlayerDebugRender::RenderCollision(&m_Player, m_Stage.GetScrollX(), m_Stage.GetScrollY(), ptr);
	//敵のデバッグ描画
	for(int i = 0;i < m_Stage.GetEnemyCount();i++)
	{
